...
public class ComponentRegistryImplTest extends ComponentRegistryTestCase {
	...
	ComponentRegistryImpl userReg = (ComponentRegistryImpl) ComponentRegistryFactoryImpl.
		getInstance().getComponentRegistry(true,
	...
	description.setUserId(USER_CREDS.getPrincipalName());
	registry = ComponentRegistryFactoryImpl.getInstance().
		getComponentRegistry(true, USER_CREDS); //user registry
	registry.register(description, testComp);
	assertEquals(1, registry.getComponentDescriptions().size());
	...
	assertEquals(1, registry.getComponentDescriptions().size());
	ComponentDescription desc = registry.getComponentDescription(description.
		getId());
	...
	assertEquals(1, registry.getComponentDescriptions().size());
	ComponentDescription result = registry.getComponentDescription(description.
		getId());
	...
	List<ComponentDescription> result = registry.getUsageInComponents(cd.
		getId());
	assertEquals(0, result.size());
	...
	ComponentRegistryImpl publicRegistry = getTestRegistry(getRegistryDir());
	ComponentRegistryImpl userRegistry = (ComponentRegistryImpl) ComponentRegistryFactoryImpl.
		getInstance().getComponentRegistry(true,
		DummyPrincipal.DUMMY_CREDENTIALS);
