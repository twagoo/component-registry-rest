...
public class GroupServiceImpl implements GroupService {
	...
	private void checkOwnership(Ownership ownership) {
		if (ownership.getComponentId() == null && ownership.getProfileId() == null)
			throw new RuntimeException("Ownership needs either a componentId or a profileId");
		if (ownership.getComponentId() != null && ownership.getProfileId() != null)
			throw new RuntimeException("Ownership has both a componentId and a profileId");
	...
	private void assertOwnershipDoesNotExist(Ownership ownership) {
		...
		o = ownershipDao.findOwnershipByGroupAndProfile(ownership.getGroupId(), ownership.getProfileId());
		if (o != null)
			throw new ValidationException("Ownership exists");
		o = ownershipDao.findOwnershipByUserAndProfile(ownership.getUserId(), ownership.getProfileId());
		if (o != null)
			throw new ValidationException("Ownership exists");
		// o =
	...
	protected boolean canUserAccessAbstractDescriptionEitherOnHisOwnOrThroughGroupMembership(RegistryUser user,
		...
		Ownership ownership = isProfile ? ownershipDao.findOwnershipByUserAndProfile(userId, description.getId())
				: ownershipDao.findOwnershipByUserAndComponent(userId, description.getId());
		if (ownership != null)
			return true;
		...
		for (Long groupId : groupIds) {
			ownership = isProfile ? ownershipDao.findOwnershipByGroupAndProfile(groupId, description.getId())
					: ownershipDao.findOwnershipByGroupAndComponent(groupId, description.getId());
			if (ownership != null)
				return true;
		}
		return false;
	...
	@Override
	public boolean canUserAccessProfileEitherOnHisOwnOrThroughGroupMembership(RegistryUser user,
			ProfileDescription profile) {
		return canUserAccessAbstractDescriptionEitherOnHisOwnOrThroughGroupMembership(user, profile);
	}

	@Override
	public boolean canUserAccessComponentEitherOnHisOwnOrThroughGroupMembership(RegistryUser user,
			ComponentDescription component) {
		return canUserAccessAbstractDescriptionEitherOnHisOwnOrThroughGroupMembership(user, component);
	}
	...
	protected void transferAbstractDescriptionOwnershipFromUserToGroup(String principal, String groupName,
			String descriptionId, boolean isProfile) {
		RegistryUser user = userDao.getByPrincipalName(principal);
		Group group = groupDao.findGroupByName(groupName);
		Ownership ownership = null;
		List<Ownership> oldOwnerships = isProfile ? ownershipDao.findOwnershipByProfileId(descriptionId) : ownershipDao
	...
	@Override
	public void transferComponentOwnershipFromUserToGroup(String principal, String groupName, String componentId) {
		transferAbstractDescriptionOwnershipFromUserToGroup(principal, groupName, componentId, false);
	}
	...
	@Override
	public void transferProfileOwnershipFromUserToGroup(String principal, String groupName, String profileId) {
		transferAbstractDescriptionOwnershipFromUserToGroup(principal, groupName, profileId, true);
	}
	...
	@Override
	public void transferItemOwnershipFromUserToGroup(String principal, long groupId, String itemId) {
		...
		if (isProfile)
			transferProfileOwnershipFromUserToGroup(principal, groupName, item.getId());
		else
			transferComponentOwnershipFromUserToGroup(principal, groupName, item.getId());
	}
